...
81. **Build identity + per-account data doc + frozen compose path
    ...
    immediately before every real PyInstaller build; the committed
    `GIT_SHA="dev"` fallback is force-added despite living in
    `.gitignore` (a deliberate, stated tradeoff — once tracked,
    gitignore no longer hides a tracked file's future modifications,
    only new untracked files). Surfaced in the window title, About
