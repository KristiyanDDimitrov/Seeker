...
def test_retag_context_menu_forces_regardless_of_checkbox(qtbot):
    ...
    window._dashboard_page._on_retag_track_clicked("t5")
...
def test_bpm_range_partial_input_blocks_the_call_with_an_error(qtbot):
    ...
    assert application.metadata_service.tag_tracks_calls == []
    assert "both" in window._dashboard_page.dashboard_notice.text().lower()
...
def test_tag_selected_with_no_selection_shows_message_and_makes_no_call(qtbot):
    ...
    assert application.metadata_service.tag_tracks_calls == []
    assert "select" in window._dashboard_page.dashboard_notice.text().lower()
...
def test_results_panel_renders_breakdown_and_per_item_reasons(qtbot):
    ...
    window._dashboard_page._render_tag_result(result)
...
def test_tag_result_notice_reports_tracks_without_art(qtbot):
    ...
    window._dashboard_page._render_tag_result({
        "tagged": 3,
    ...
    assert not window._dashboard_page.dashboard_notice.isHidden()
    assert "2 without cover art" in window._dashboard_page.dashboard_notice.text()
...
def test_tag_result_notice_shows_success_when_everything_worked(qtbot):
    ...
    assert not window._dashboard_page.dashboard_notice.isHidden()
    assert "Tagged 5 tracks" in window._dashboard_page.dashboard_notice.text()
    assert "without cover art" not in window._dashboard_page.dashboard_notice.text()
...
def test_tag_result_notice_reports_already_tagged_with_nothing_else_done(
    ...
    assert not window._dashboard_page.dashboard_notice._action_button.isHidden()
    assert (
        window._dashboard_page.dashboard_notice._action_button.text()
...
def test_tag_result_notice_fix_art_action_triggers_fix_missing_art(
    ...
    window._dashboard_page.dashboard_notice._action_button.click()
...
def test_fix_missing_art_button_calls_service_and_shows_result(qtbot):
    ...
    qtbot.waitUntil(
        ...
        and "Fixed art for 3" in window._dashboard_page.dashboard_notice.text(),
        timeout=2000,
    )
    text = window._dashboard_page.dashboard_notice.text()
...
def test_fill_missing_art_urls_button_reports_the_real_count(qtbot):
    ...
    qtbot.waitUntil(
        lambda: not window._dashboard_page.dashboard_notice.isHidden()
        and "7" in window._dashboard_page.dashboard_notice.text(),
        timeout=2000,
    )
    assert (
...
def test_fill_missing_art_urls_button_reports_zero_found(qtbot):
    ...
    qtbot.waitUntil(
        lambda: not window._dashboard_page.dashboard_notice.isHidden()
        and "No missing" in window._dashboard_page.dashboard_notice.text(),
        timeout=2000,
    )
...
def test_rename_files_button_confirmed_calls_apply_and_shows_result(
    ...
    qtbot.waitUntil(
        lambda: not window._dashboard_page.dashboard_notice.isHidden()
        and "Renamed 1" in window._dashboard_page.dashboard_notice.text(),
        timeout=2000,
    )
...
def test_rename_result_notice_names_files_whose_written_name_differed(
    ...
    qtbot.waitUntil(
        lambda: not window._dashboard_page.dashboard_notice.isHidden()
        and "Renamed 2" in window._dashboard_page.dashboard_notice.text(),
        timeout=2000,
    )
    text = window._dashboard_page.dashboard_notice.text()
